...
export default function RelationshipExplorerPage({ user, toast }: Props) {
  ...
  const loadParents = async (relation: any) => {
    ...
    try {
      const response = await api.get(`/api/tables/${encodeURIComponent(relation.to_table)}/records?limit=500&offset=0`, user.role);
  ...
  const openParent = async (row: any) => {
    ...
    try {
      const response = await api.get(`/api/tables/${encodeURIComponent(activeRelation.from_table)}/records?limit=500&offset=0`, user.role);
