...
def test_phase_sixteen_status_is_public_and_never_accepts_raw_sql():
    with patch("app.services.demo_readiness_service.get_database_health", return_value=SimpleNamespace(connected=True, message="ok")), patch("app.services.demo_readiness_service.llm_service.check_llm_health", return_value=SimpleNamespace(connected=True, model_installed=True, message="ok")), patch("app.services.demo_readiness_service.document_rag_service.status", return_value={"chromadb_available": True}):
