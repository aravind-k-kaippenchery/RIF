...
def test_generate_json_sends_pydantic_schema_and_validates_response():
    ...
    # Ollama receives a grammar-compatible schema with annotation-only keywords
    # removed. The full Pydantic schema remains in the prompt and the returned
    # object is still validated by Pydantic after generation.
    assert captured[0]["format"] == service._ollama_compatible_schema(
        IntentClassificationResult.model_json_schema()
    )
    assert captured[0]["format"]["type"] == "object"
    assert set(captured[0]["format"]["required"]) == {"route", "requires_clarification"}
