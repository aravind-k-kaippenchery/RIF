...
import {
  ...
  Database, FileCode2, FileSearch, FileText, FolderOpen, History,
  LayoutDashboard, ListTree, LoaderCircle, LockKeyhole, LogOut, Menu,
  MessageCircle, MessageSquareText, Network, PanelRight, Play, RefreshCw,
  Search, Send, ShieldCheck, Sparkles, Table2, Upload,
...
import RelationshipExplorerPage from './pages/RelationshipExplorer';
import DuplicateCenterPage from './pages/DuplicateCenter';

const STORAGE_KEY = 'b2b-intelligence.local-user';
const SESSION_KEY = 'b2b-intelligence.session-id';

export type Page = 'dashboard' | 'assistant' | 'explorer' | 'documents' | 'audit' | 'demo';
...
const pageMeta: Record<Page, { title: string; subtitle: string; icon: typeof LayoutDashboard; admin?: boolean }> = {
  dashboard: { title: 'Command Center', subtitle: 'Health, metrics, and workspace readiness', icon: LayoutDashboard },
  assistant: { title: 'AI Assistant', subtitle: 'Query, RAG, safe actions, sessions, and evidence in one workspace', icon: Bot },
  explorer: { title: 'Data Explorer', subtitle: 'Browse records, schema, and verified relationships', icon: Table2 },
  documents: { title: 'Documents & OCR', subtitle: 'Upload, extract, index, and inspect document chunks', icon: FolderOpen },
  audit: { title: 'Activity & Audit', subtitle: 'Trace prompts, confirmations, actions, and recovery evidence', icon: ClipboardList, admin: true },
  demo: { title: 'Demo Center', subtitle: 'Hidden reviewer route for guided capability proof', icon: Sparkles, admin: true },
};

const nav: { page: Page; label: string; icon: typeof LayoutDashboard; admin?: boolean }[] = [
  { page: 'dashboard', label: 'Command Center', icon: LayoutDashboard },
  { page: 'assistant', label: 'AI Assistant', icon: Bot },
  { page: 'documents', label: 'Documents & OCR', icon: FolderOpen },
  { page: 'explorer', label: 'Data Explorer', icon: Table2 },
  { page: 'audit', label: 'Activity & Audit', icon: ClipboardList, admin: true },
];
...
function App() {
  ...
  const [loginGate, setLoginGate] = useState<{ userName: string } | null>(null);

  const beginLoginGate = (nextUser: LocalUser) => {
    setLoginGate({ userName: nextUser.name || 'User' });
  };
  ...
  const login = (nextUser: LocalUser) => {
    ...
    beginLoginGate(nextUser);
    window.setTimeout(() => {
      setUser(nextUser);
      setPage('dashboard');
      pushToast(`Workspace ready. Welcome, ${nextUser.name.split(' ')[0] || 'there'}.`, 'success');
      setLoginGate(null);
    }, 1400);
  };
  ...
  if (!user) return <LoginView onEnter={login} />;

  if (loginGate) {
    return (
      <div className="app-shell app-shell--gate">
        <TransitionGate
          username={loginGate.userName}
          onDone={() => {
            // After transition, show the existing gate for a split second to preserve existing UX.
            // LoadingGate already includes animated progress.
            setLoginGate(null);
          }}
        />
      </div>
  ...
  return (
    <div className="app-shell">
      ...
      <Sidebar user={user} page={page} onPage={setPage} open={sidebarOpen} onToggle={() => setSidebarOpen((value) => !value)} onLogout={logout} />
      <main className="main-area">
        <Topbar user={user} page={page} sidebarOpen={sidebarOpen} onMenu={() => setSidebarOpen((value) => !value)} onPage={setPage} />
        <section className="page-stage">
          <PageTransition key={page}>
            <PageRenderer page={page} user={user} onPage={setPage} toast={pushToast} />
          </PageTransition>
        </section>
...
function LoginView({ onEnter }: { onEnter: (user: LocalUser) => void }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setBootReady(true), 2100);
  ...
  const enter = (event: FormEvent) => {
    ...
    if (!name.trim() || !email.trim()) {
      setError('Please add your name and email before entering the workspace.');
      return;
    }
    setError('');
    setLaunching(true);
    window.setTimeout(() => onEnter({ name: name.trim(), email: email.trim(), role }), 850);
  };

  return (
    <div className={`login-page ${launching ? 'is-launching' : ''}`}>
      <div className="login-photo" aria-hidden="true" />
      <div className="login-photo-wash" aria-hidden="true" />
      ...
      <div className="message-pulse" aria-hidden="true"><MessageCircle size={18} /><span>secure local message stream</span></div>
      <div className="phone-signal" aria-hidden="true"><span /><span /><span /></div>

      <div className="login-content">
        <section className="login-story">
          <div className="brand-row fade-rise delay-1">
            <LogoMark />
            <div className="brand-word">B2B <span>Intelligence</span></div>
          </div>
          <div className="eyebrow fade-rise delay-2"><i /> LOCAL AGENTIC AI WORKSPACE</div>
          <h1 className="login-title">
            <span className="mask-line"><span>Find signal in</span></span>
            <span className="mask-line"><span>your <em>business data.</em></span></span>
          ...
          <div className="trust-stack fade-rise delay-6">
            ...
            <Trust text="Evidence-grounded intelligence" icon={<FileSearch size={14} />} />
            <Trust text="Confirmation-protected actions" icon={<ShieldCheck size={14} />} />
          </div>
          <div className="boot-console fade-rise delay-7" aria-live="polite">
            <BootLine ready={bootReady} text="LOCAL ENVIRONMENT DETECTED" index={0} />
            <BootLine ready={bootReady} text="INITIALIZING AGENT ROUTER" index={1} />
            <BootLine ready={bootReady} text="VERIFYING GROUNDED DATA PATHS" index={2} />
            <BootLine ready={bootReady} text={bootReady ? 'LOCAL SERVICES READY' : 'PREPARING LOCAL WORKSPACE'} index={3} />
          </div>
        </section>

        <form className="access-card" onSubmit={enter}>
          <div className="card-glow" />
          <div className="access-kicker"><Sparkles size={14} /> LOCAL WORKSPACE ACCESS</div>
          <h2>Enter your workspace.</h2>
          <p>Access your local AI business intelligence environment.</p>
          <label className="field animated-field">
            <span>Full name</span>
          ...
          <div className="role-label">Workspace role</div>
          <div className="role-grid">
            <RoleCard selected={role === 'normal_user'} onClick={() => setRole('normal_user')} icon={<UserRound size={19} />} title="Normal User" description="Search data and inspect evidence." />
            <RoleCard selected={role === 'admin'} onClick={() => setRole('admin')} icon={<ShieldCheck size={19} />} title="Admin" description="Audit, rollback and benchmark access." />
          </div>
...
function Trust({ icon, text }: { icon: ReactNode; text: string }) {
  return <div className="trust-item"><b>{icon}</b><span>{text}</span></div>;
}

function RoleCard({ icon, title, description, selected, onClick }: { icon: ReactNode; title: string; description: string; selected: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`role-card ${selected ? 'selected' : ''}`}><span className="role-icon">{icon}</span><b>{title}</b><small>{description}</small></button>;
}

function Sidebar({ user, page, onPage, open, onToggle, onLogout }: { user: LocalUser; page: Page; onPage: (page: Page) => void; open: boolean; onToggle: () => void; onLogout: () => void }) {
  return <aside className={`sidebar ${open ? 'open' : 'closed'}`}>
    <div className="side-top">
      <button className="brand-button" onClick={() => onPage('dashboard')} aria-label="Open Command Center"><LogoMark /><span>B2B <b>Intelligence</b></span></button>
      <button className="collapse-button" onClick={onToggle} aria-label="Collapse menu"><Menu size={18} /></button>
    </div>
    <nav className="side-nav">
      <div className="nav-label">WORKSPACE</div>
      {nav.map((item) => {
        const Icon = item.icon;
        const locked = item.admin && user.role !== 'admin';
        return <button key={item.page} onClick={() => !locked && onPage(item.page)} className={`nav-item ${page === item.page ? 'active' : ''} ${locked ? 'locked' : ''}`} title={locked ? 'Admin Access Required' : item.label}>
          <Icon size={18} /> <span>{item.label}</span>{item.admin && <small>{locked ? <LockKeyhole size={12} /> : 'ADMIN'}</small>}
        </button>;
      })}
    </nav>
    <div className="side-bottom">
      <div className="mini-local"><span className="online-dot" /> <span>LOCAL MODE</span></div>
      <button className="profile-card" onClick={() => onPage('dashboard')}>
        <span className="avatar">{user.name.slice(0, 1).toUpperCase()}</span>
        <span><b>{user.name}</b><small>{user.role === 'admin' ? 'Admin Workspace' : 'Normal Workspace'}</small></span>
        <ChevronDown size={15} />
      </button>
      <button className="logout" onClick={onLogout}><LogOut size={16} /> <span>Log out</span></button>
    </div>
  </aside>;
}

function Topbar({ user, page, sidebarOpen, onMenu, onPage }: { user: LocalUser; page: Page; sidebarOpen: boolean; onMenu: () => void; onPage: (page: Page) => void }) {
  const meta = pageMeta[page];
  return <header className="topbar">
    <div className="title-block"><button className="mobile-menu" onClick={onMenu}><Menu size={18} /></button><div><h1>{meta.title}</h1><p>{meta.subtitle}</p></div></div>
...
function PageRenderer({ page, user, onPage, toast }: { page: Page; user: LocalUser; onPage: (page: Page) => void; toast: (message: string, type?: Toast['type']) => void }) {
  if (pageMeta[page].admin && user.role !== 'admin') return <AdminRequired onReturn={() => onPage('dashboard')} />;
  if (page === 'dashboard') return <Dashboard user={user} onPage={onPage} toast={toast} />;
  if (page === 'assistant') return <Assistant user={user} toast={toast} />;
  if (page === 'explorer') return <Explorer user={user} toast={toast} />;
  if (page === 'documents') return <DocumentsPage user={user} toast={toast} />;
  if (page === 'audit') return <AuditPage user={user} toast={toast} />;
  if (page === 'demo') return <DemoPage user={user} toast={toast} />;
  return <Dashboard user={user} onPage={onPage} toast={toast} />;
}

function Dashboard({ user, onPage, toast }: { user: LocalUser; onPage: (page: Page) => void; toast: (message: string, type?: Toast['type']) => void }) {
  const [loading, setLoading] = useState(true);
  const [errors, setErrors] = useState<string[]>([]);
  const [data, setData] = useState<Record<string, any>>({});

  // realtime health flags: if an endpoint responds, show Connected; otherwise Not connected
  ...
  const load = async () => {
    ...
    if (user.role === 'admin') calls.push(['benchmarks', '/api/benchmarks/summary?limit=250']);

    const results = await Promise.allSettled(
      calls.map(async ([name, url]) => [name, await api.get(url, user.role)] as const),
    );
    ...
    results.forEach((result) => {
      if (result.status === 'fulfilled') {
        ...
        next[name] = result.value[1].data;
        if (name === 'demo' || name === 'features') flags.fastapi = 'connected';
        if (name === 'db') flags.postgres = 'connected';
        if (name === 'rag') flags.chromadb = 'connected';
  ...
  const services = [
    ['FastAPI', 'Backend request surface', healthFlags.fastapi === 'connected' ? 'Connected' : 'Not connected', Code2],
    ['PostgreSQL', 'Structured business data', healthFlags.postgres === 'connected' ? 'Connected' : 'Not connected', Database],
    ['Ollama', 'Local model runtime', healthFlags.ollama === 'connected' ? 'Connected' : 'Not connected', Bot],
    ['ChromaDB', 'Local document knowledge', healthFlags.chromadb === 'connected' ? 'Connected' : 'Not connected', BrainCircuit],
    ['LangGraph', 'Route orchestration', healthFlags.agent === 'connected' ? 'Connected' : 'Not connected', Network],
    ['MCP', 'Controlled tool boundary', healthFlags.mcp === 'connected' ? 'Connected' : 'Not connected', Boxes],
  ];
  const features: any[] = Array.isArray(data.features?.features) ? data.features.features : [];
  const counts = asRecord(data.db?.counts || data.db?.summary || data.db);
  const docs = Array.isArray(data.documents?.documents) ? data.documents.documents : [];
  const benchmarkSummary = asRecord(data.benchmarks);
  const benchmarkMetrics = Object.entries(benchmarkSummary.metrics || benchmarkSummary.summary || benchmarkSummary)
    .filter(([key]) => !['runs', 'phase', 'raw_sql_accepted'].includes(key))
    .slice(0, 4);
  ...
  return <div className="dashboard page-enter">
    ...
    {user.role === 'admin' && <section className="surface-card dashboard-benchmarks"><SectionHeading title="Quality snapshot" eyebrow="BENCHMARK METRICS MERGED INTO DASHBOARD" />{loading ? <div className="metric-grid">{Array.from({ length: 4 }).map((_, index) => <div className="metric-card skeleton-card" key={index} />)}</div> : benchmarkMetrics.length ? <div className="metric-grid">{benchmarkMetrics.map(([key, value]) => <MetricCard key={key} label={prettyKey(key)} value={typeof value === 'object' ? stringValue((value as any).average || (value as any).value || 'Available') : stringValue(value)} />)}</div> : <Empty icon={<BarChart3 size={22} />} title="No benchmark summary yet" text="Run benchmark scenarios through the backend API when reviewer evidence is required." />}</section>}
  </div>;
}

function Assistant({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  const [workspace, setWorkspace] = useState<'ask' | 'knowledge' | 'actions' | 'sessions' | 'evidence'>('ask');
  const [actionView, setActionView] = useState<'queue' | 'duplicates'>('queue');
  const [messages, setMessages] = useState<{ id: string; from: 'user' | 'assistant'; text: string; response?: ApiEnvelope<any> }[]>([]);
  ...
  const submit = async (event?: FormEvent, quick?: string) => {
    event?.preventDefault();
    const prompt = (quick || question).trim();
    if (!prompt || loading) return;
    setQuestion(''); setLoading(true); setMessages((list) => [...list, { id: `user-${Date.now()}`, from: 'user', text: prompt }]);
    try {
  ...
  const mutatePending = async (response: ApiEnvelope<any>, action: 'confirm' | 'cancel') => {
    const id = response.pending_action_id;
    if (!id || !sessionId) return;
    try {
      const result = await api.post(`/api/crud/actions/${id}/${action}`, user.role, { session_id: sessionId });
      setMessages((list) => [...list, { id: result.request_id || `${action}-${Date.now()}`, from: 'assistant', text: result.answer || `Action ${action}ed.`, response: result }]);
      toast(result.answer || `Action ${action}ed.`, action === 'confirm' ? 'success' : 'info');
      setConfirming(null);
    } catch (error) { toast(error instanceof Error ? error.message : 'The stored action could not be updated.', 'error'); }
  };

  return <div className="assistant-page page-enter">
    <div className="assistant-toolbar"><div><div className="eyebrow compact"><i /> CONSOLIDATED INTELLIGENCE WORKSPACE</div><h2>Ask, retrieve, review, and act.</h2><p>Natural-language querying, document RAG, confirmation-gated writes, duplicate checks, sessions, and pipeline evidence now live together.</p></div><div className="assistant-tags"><StatusBadge status="Local AI" /><StatusBadge status="Source-backed" /><StatusBadge status="Safe actions" /></div></div>
    <div className="tab-row workspace-tabs">
      <button className={workspace === 'ask' ? 'selected' : ''} onClick={() => setWorkspace('ask')}><Bot size={14} /> Ask</button>
      <button className={workspace === 'knowledge' ? 'selected' : ''} onClick={() => setWorkspace('knowledge')}><FileSearch size={14} /> Document search</button>
      <button className={workspace === 'actions' ? 'selected' : ''} onClick={() => setWorkspace('actions')}><ListTree size={14} /> Safe actions</button>
      <button className={workspace === 'sessions' ? 'selected' : ''} onClick={() => setWorkspace('sessions')}><History size={14} /> Sessions</button>
      <button className={workspace === 'evidence' ? 'selected' : ''} onClick={() => setWorkspace('evidence')}><Network size={14} /> Pipeline evidence</button>
    </div>
    ...
    {workspace === 'ask' && <>
      ...
      {confirming && <ConfirmModal response={confirming} onClose={() => setConfirming(null)} onConfirm={() => void mutatePending(confirming, 'confirm')} />}
    </>}

    {workspace === 'knowledge' && <KnowledgeWorkspace user={user} toast={toast} />}
    {workspace === 'actions' && <div className="embedded-workspace"><div className="tab-row sub-tabs"><button className={actionView === 'queue' ? 'selected' : ''} onClick={() => setActionView('queue')}>CRUD proposals</button><button className={actionView === 'duplicates' ? 'selected' : ''} onClick={() => setActionView('duplicates')}>Duplicate-aware bulk insert</button></div>{actionView === 'queue' ? <CrudQueuePage user={user} toast={toast} /> : <DuplicateCenterPage user={user} toast={toast} />}</div>}
    {workspace === 'sessions' && <div className="embedded-workspace"><SessionCenterPage user={user} toast={toast} /></div>}
    {workspace === 'evidence' && <div className="embedded-workspace"><PipelineConsolePage user={user} toast={toast} /></div>}
  </div>;
}
...
function ConfirmModal({ response, onClose, onConfirm }: { response: ApiEnvelope<any>; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-backdrop"><div className="confirm-modal"><span className="confirm-icon"><AlertTriangle size={24} /></span><div className="eyebrow compact"><i /> EXPLICIT CONFIRMATION REQUIRED</div><h3>Confirm database change?</h3><p>You are about to execute one stored, validator-approved action. The action and its outcome will be recorded in audit history.</p><div className="confirm-summary"><span>Action <b>{action.action_type || 'write'}</b></span><span>Target <b>{action.target_table || 'business data'}</b></span><span>Pending ID <b>{response.pending_action_id || '—'}</b></span></div><div className="modal-actions"><button className="button secondary" onClick={onClose}>Go back</button><button className="button warning" onClick={onConfirm}>Confirm and execute <ShieldCheck size={16} /></button></div></div></div>;
}

function Explorer({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  const [view, setView] = useState<'data' | 'schema' | 'relationships'>('data');
  const [tables, setTables] = useState<any[]>([]); const [table, setTable] = useState('employees'); const [records, setRecords] = useState<any[]>([]); const [tableInfo, setTableInfo] = useState<Record<string, any>>({}); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [search, setSearch] = useState(''); const [selected, setSelected] = useState<Record<string, any> | null>(null); const [refreshKey, setRefreshKey] = useState(0);
  useEffect(() => { void (async () => { try { const directory = await api.get('/api/tables', user.role); const entries = (directory.data?.tables || directory.data?.table_directory || []) as any[]; setTables(entries); if (entries[0]?.table_name) setTable(entries.find((entry: any) => entry.table_name === 'employees')?.table_name || entries[0].table_name); } catch (e) { setError(e instanceof Error ? e.message : 'Could not load the table directory.'); } finally { setLoading(false); } })(); }, [user.role]);
  useEffect(() => {
    if (!table) return;
    void (async () => {
      setLoading(true);
      setError('');
      try {
        const rows = await api.get(`/api/tables/${encodeURIComponent(table)}/records?limit=50&offset=0`, user.role);
        setRecords((rows.data?.rows || rows.data?.records || []) as any[]);

        // Schema metadata is useful but must never prevent valid records from rendering.
        try {
          const schema = await api.get(`/api/schema/tables/${encodeURIComponent(table)}`, user.role);
          setTableInfo(asRecord(schema.data));
        } catch {
          setTableInfo({});
        }
      } catch (e) {
  ...
  }, [table, user.role, refreshKey]);
  const filtered = records.filter((row) => JSON.stringify(row).toLowerCase().includes(search.toLowerCase()));
  return <div className="explorer-page page-enter">
    <div className="tab-row workspace-tabs"><button className={view === 'data' ? 'selected' : ''} onClick={() => setView('data')}><Table2 size={14} /> Data</button><button className={view === 'schema' ? 'selected' : ''} onClick={() => setView('schema')}><Boxes size={14} /> Schema</button><button className={view === 'relationships' ? 'selected' : ''} onClick={() => setView('relationships')}><UsersRound size={14} /> Relationships</button></div>
    {view === 'data' && <><div className="explorer-layout"><aside className="table-directory surface-card"><SectionHeading title="Table directory" eyebrow="CONTROLLED VIEWS" />{loading && !tables.length ? <SkeletonLines /> : tables.map((item: any) => { const name = item.table_name || item.name || String(item); const restricted = Boolean(item.requires_admin_for_records || item.admin_only); return <button key={name} className={`table-option ${table === name ? 'selected' : ''} ${restricted && user.role !== 'admin' ? 'locked' : ''}`} onClick={() => { if (restricted && user.role !== 'admin') { toast('Admin access is required for operational table records.', 'error'); return; } setTable(name); }}><Database size={16} /><span>{name}</span>{restricted ? <LockKeyhole size={13} /> : <ChevronRight size={14} />}</button>; })}</aside><section className="table-main surface-card"><div className="table-top"><div><div className="eyebrow compact"><i /> BOUNDED FRONTEND READ</div><h2>{prettyKey(table)}</h2><p>{tableInfo.table?.description || tableInfo.description || 'Live records through a controlled MCP-backed table view.'}</p></div><div className="table-tools"><div className="search-box"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search visible records" /></div><button className="icon-button" onClick={() => setRefreshKey((value) => value + 1)}><RefreshCw size={16} /></button></div></div>{error ? <div className="inline-alert danger"><AlertTriangle size={16} /> {error}</div> : null}{loading ? <TableSkeleton /> : <RecordTable rows={filtered} onRow={setSelected} />}</section></div>{selected && <RecordDrawer row={selected} onClose={() => setSelected(null)} />}</>}
    {view === 'schema' && <div className="embedded-workspace"><SchemaGraphPage user={user} toast={toast} /></div>}
    {view === 'relationships' && <div className="embedded-workspace"><RelationshipExplorerPage user={user} toast={toast} /></div>}
  </div>;
}

function DocumentsPage({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  const [docs, setDocs] = useState<any[]>([]); const [loading, setLoading] = useState(true); const [uploading, setUploading] = useState(false); const [selected, setSelected] = useState<any | null>(null); const [details, setDetails] = useState<ApiEnvelope<any> | null>(null);
  const load = async () => { setLoading(true); try { const response = await api.get('/api/documents?limit=100', user.role); setDocs((response.data?.documents || []) as any[]); } catch (e) { toast(e instanceof Error ? e.message : 'Could not load documents.', 'error'); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, [user.role]);
...
function KnowledgeWorkspace({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  const [mode, setMode] = useState<'retrieve' | 'answer'>('answer'); const [question, setQuestion] = useState(''); const [result, setResult] = useState<ApiEnvelope<any> | null>(null); const [loading, setLoading] = useState(false); const [status, setStatus] = useState<Record<string, any>>({});
...
function AuditPage({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  const [tab, setTab] = useState<'logs' | 'actions'>('actions'); const [logs, setLogs] = useState<Record<string, any>>({}); const [actions, setActions] = useState<any[]>([]); const [selected, setSelected] = useState<any | null>(null); const [detail, setDetail] = useState<ApiEnvelope<any> | null>(null); const [loading, setLoading] = useState(true);
...
function DemoPage({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  const [data, setData] = useState<Record<string, any>>({}); const [loading, setLoading] = useState(true); const [smoke, setSmoke] = useState<ApiEnvelope<any> | null>(null); const [running, setRunning] = useState(false);
  const load = async () => { setLoading(true); try { const [status, features, scenarios, readiness, report] = await Promise.all([api.get('/api/demo/status', user.role), api.get('/api/demo/features', user.role), api.get('/api/demo/scenarios', user.role), api.get('/api/demo/readiness', user.role), api.get('/api/demo/report', user.role)]); setData({ status: status.data, features: features.data, scenarios: scenarios.data, readiness: readiness.data, report: report.data }); } catch (e) { toast(e instanceof Error ? e.message : 'Could not load the demo center.', 'error'); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, [user.role]);
...
function ServiceCard({ title, detail, state, icon, loading }: { title: string; detail: string; state: string; icon: ReactNode; loading?: boolean }) { return <article className="service-card">{loading ? <div className="skeleton-line short" /> : <><span className="service-icon">{icon}</span><div className="service-state"><StatusBadge status={state} /></div><h3>{title}</h3><p>{detail}</p><small><span className={`status-dot ${statusTone(state)}`} /> {state}</small></>}</article>; }
function CapabilityTile({ title, status }: { title: string; status: string }) { return <div className="capability-tile"><span><Check size={14} /></span><div><b>{title}</b><small>{status}</small></div></div>; }
function QuickAction({ icon, title, text, onClick }: { icon: ReactNode; title: string; text: string; onClick: () => void }) { return <button className="quick-action" onClick={onClick}><span>{icon}</span><b>{title}</b><small>{text}</small><ChevronRight size={16} /></button>; }
export function SectionHeading({ title, eyebrow, action }: { title: string; eyebrow?: string; action?: ReactNode }) { return <div className="section-heading"><div>{eyebrow && <div className="eyebrow compact"><i /> {eyebrow}</div>}<h3>{title}</h3></div>{action}</div>; }
export function StatusBadge({ status }: { status: string }) { return <span className={`status-badge ${statusTone(status)}`}>{status}</span>; }
export function RouteBadge({ route }: { route?: string | null }) { const label = (route || 'system').replace(/_/g, ' '); return <span className="route-badge"><Network size={12} /> {label}</span>; }
export function MetricCard({ label, value }: { label: string; value: string }) { return <div className="metric-card"><span>{label}</span><b>{value}</b><small><Activity size={12} /> Latest local benchmark</small></div>; }
export function Empty({ icon, title, text }: { icon: ReactNode; title: string; text: string }) { return <div className="empty-state"><span>{icon}</span><h3>{title}</h3><p>{text}</p></div>; }
export function SkeletonLines() { return <div className="skeleton-lines"><i/><i/><i/><i/></div>; }
...
export function DataPreview({ rows }: { rows: any[] }) { if (!rows.length) return null; const columns = Object.keys(rows[0]).slice(0, 4); return <div className="data-preview"><div className="preview-head">{columns.map((column) => <span key={column}>{prettyKey(column)}</span>)}</div>{rows.map((row, index) => <div className="preview-row" key={index}>{columns.map((column) => <span key={column}>{stringValue(row[column])}</span>)}</div>)}</div>; }
