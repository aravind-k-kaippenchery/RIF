...
function Assistant({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  ...
  return <div className="assistant-page page-enter">
    <div className="assistant-toolbar"><div><div className="eyebrow compact"><i /> FOCUSED ASSISTANT WORKSPACE</div><h2>Ask and search documents.</h2><p>The frontend now exposes only the main assistant chat and document-search experience. Safe-action confirmations still appear inside Ask only when the backend returns a pending action.</p></div><div className="assistant-tags"><StatusBadge status="Local AI" /><StatusBadge status="Source-backed" /></div></div>
...
function ConfirmModal({ response, onClose, onConfirm }: { response: ApiEnvelope<any>; onClose: () => void; onConfirm: () => void }) {
  ...
  const dangerousAction = actionType.includes('delete');
  return <div className="modal-backdrop"><div className="confirm-modal"><span className="confirm-icon"><AlertTriangle size={24} /></span><div className="eyebrow compact"><i /> {dangerousAction ? 'DANGEROUS DELETE CONFIRMATION' : 'EXPLICIT CONFIRMATION REQUIRED'}</div><h3>{dangerousAction ? 'Confirm dangerous delete?' : 'Confirm database change?'}</h3><p>{dangerousAction ? 'This stored action deletes data. Verify the exact target and record count before execution. The outcome will be recorded in audit history.' : 'You are about to execute one stored, validator-approved action. The action and its outcome will be recorded in audit history.'}</p><div className="confirm-summary"><span>Action <b>{action.action_type || 'write'}</b></span><span>Target <b>{action.target_table || 'business data'}</b></span><span>Pending ID <b>{response.pending_action_id || 'â€”'}</b></span></div><div className="modal-actions"><button className="button secondary" onClick={onClose}>Go back</button><button className="button warning" onClick={onConfirm}>Confirm and execute <ShieldCheck size={16} /></button></div></div></div>;
}
...
function Explorer({ user, toast }: { user: LocalUser; toast: (message: string, type?: Toast['type']) => void }) {
  ...
  const [pageMeta, setPageMeta] = useState({ rowCount: 0, totalRowCount: 0, hasMore: false, previousOffset: null as number | null, nextOffset: null as number | null, limit: 100 });
  const pageSize = 100;
  ...
  useEffect(() => { setOffset(0); setSearch(''); }, [table]);
  useEffect(() => {
    if (!table) return;
    void (async () => {
      setLoading(true);
      setError('');
      try {
        const rows = await api.get(`/api/tables/${encodeURIComponent(table)}/records?limit=${pageSize}&offset=${offset}`, user.role);
        const data = asRecord(rows.data);
        setRecords((data.rows || data.records || []) as any[]);
        setPageMeta({
          rowCount: Number(data.row_count || data.returned_row_count || 0),
          totalRowCount: Number(data.total_row_count || data.row_count || 0),
          hasMore: Boolean(data.has_more),
          previousOffset: typeof data.previous_offset === 'number' ? data.previous_offset : null,
        ...
        setRecords([]);
        setPageMeta({ rowCount: 0, totalRowCount: 0, hasMore: false, previousOffset: null, nextOffset: null, limit: pageSize });
        setError(e instanceof Error ? e.message : 'Could not load records.');
      } finally {
        setLoading(false);
      }
    })();
  }, [table, user.role, refreshKey, offset]);
  const filtered = records.filter((row) => JSON.stringify(row).toLowerCase().includes(search.toLowerCase()));
  const rangeStart = pageMeta.totalRowCount === 0 ? 0 : offset + 1;
  ...
  return <div className="explorer-page page-enter">
    ...
    {view === 'data' && <><div className="explorer-layout"><aside className="table-directory surface-card"><SectionHeading title="Table directory" eyebrow="CONTROLLED VIEWS" />{loading && !tables.length ? <SkeletonLines /> : tables.map((item: any) => { const name = item.table_name || item.name || String(item); const restricted = Boolean(item.requires_admin_for_records || item.admin_only); const countText = typeof item.total_row_count === 'number' ? `${item.total_row_count}` : ''; return <button key={name} className={`table-option ${table === name ? 'selected' : ''} ${restricted && user.role !== 'admin' ? 'locked' : ''}`} onClick={() => { if (restricted && user.role !== 'admin') { toast('Admin access is required for operational table records.', 'error'); return; } setTable(name); }}><Database size={16} /><span>{name}</span>{countText ? <small>{countText}</small> : null}{restricted ? <LockKeyhole size={13} /> : <ChevronRight size={14} />}</button>; })}</aside><section className="table-main surface-card"><div className="table-top"><div><div className="eyebrow compact"><i /> PAGED FRONTEND READ</div><h2>{prettyKey(table)}</h2><p>{tableInfo.table?.description || tableInfo.description || 'Live records through a controlled MCP-backed table view.'}</p><small className="muted-copy">Showing {rangeStart}-{rangeEnd} of {pageMeta.totalRowCount || records.length} record(s). Search filters only the visible page.</small></div><div className="table-tools"><div className="search-box"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search visible page" /></div><button className="icon-button" onClick={() => setRefreshKey((value) => value + 1)}><RefreshCw size={16} /></button></div></div><div className="pagination-row"><button className="button small secondary" disabled={offset === 0 || loading} onClick={() => setOffset(Math.max(0, offset - pageSize))}>Previous</button><span>Page {Math.floor(offset / pageSize) + 1}</span><button className="button small secondary" disabled={!pageMeta.hasMore || loading} onClick={() => setOffset(pageMeta.nextOffset ?? offset + pageSize)}>Next</button></div>{error ? <div className="inline-alert danger"><AlertTriangle size={16} /> {error}</div> : null}{loading ? <TableSkeleton /> : <RecordTable rows={filtered} onRow={setSelected} />}</section></div>{selected && <RecordDrawer row={selected} onClose={() => setSelected(null)} />}</>}
