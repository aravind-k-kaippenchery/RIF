...
def frontend_table_records(
    ...
    return ResponseBuilder.success(
        request,
        answer=f"Frontend records viewer returned {result['row_count']} record(s) from {result.get('total_row_count', result['row_count'])} total record(s) from '{result['table_name']}'.",
        data=result,
