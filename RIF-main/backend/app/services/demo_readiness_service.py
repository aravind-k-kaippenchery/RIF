...
class DemoReadinessService:
    ...
    def public_status(self) -> dict[str, Any]:
        ...
        fallback = llm_service.fallback_model_health()
        ...
        if fallback is not None:
            # Informational only: the fallback model is best-effort resilience, not a
            # required capability, so its absence does not block full_demo_ready.
            checks.append(
                _CapabilityCheck(
                    "Local fallback model",
                    bool(fallback.connected and fallback.model_installed),
                    "ready" if fallback.connected and fallback.model_installed else "attention",
                    f"Fallback model '{fallback.configured_model}' is used only if the primary local model is unavailable. {fallback.message}",
                )
            )
        return {
            "phase": self.PHASE,
    ...
    def final_report(self, db: DbSession) -> dict[str, Any]:
        ...
        limitations = [
            ...
            "An optional local fallback model (see docs/LOCAL_FALLBACK_MODEL.md) adds resilience if the primary model is not installed or is slow, but the system still has no multi-machine or cloud-backed high-availability path -- if Ollama itself is down, or every configured local model is unavailable, generation still fails by design.",
        ]
        return {
            "phase": self.PHASE,
