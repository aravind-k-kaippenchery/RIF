...
class LLMService:
    ...
    def check_llm_health(self, model_name: str | None = None) -> OllamaHealth:
        """Check local service reachability and whether the given model is already pulled.

        Defaults to the primary configured model. Pass ``model_name`` to check the
        configured local fallback model instead -- both checks hit the same loopback
        Ollama server, so this never involves a network call outside the machine.
        """
        ...
        try:
            ...
            installed = any(self._model_matches(target_model, name) for name in names)
            message = (
    ...
    def fallback_model_health(self) -> OllamaHealth | None:
        """Return health for the configured local fallback model, or None if fallback is off."""

        settings = get_settings()
        if not settings.ollama_fallback_enabled or not settings.ollama_fallback_model.strip():
            return None
        return self.check_llm_health(model_name=settings.ollama_fallback_model)

    def _ensure_model_ready(self, model_name: str) -> OllamaHealth:
        health = self.check_llm_health(model_name=model_name)
    ...
    def _generate_json_with_model(
        ...
        *,
        ...
        """Generate and validate one Pydantic-constrained JSON result against one named local model.
        ...
        for attempt in range(1, settings.ollama_max_attempts + 1):
            ...
            try:
                ...
                raise OllamaUnavailableError(
                    ...
                    message=(
                        f"Local Ollama model '{model_name}' timed out while generating the structured response. "
                ...
                log_event(level="WARNING", event="ollama_request_error", model=model_name, error_type=type(exc).__name__)
                raise OllamaUnavailableError(
        ...
        raise LLMOutputValidationError(
            ...
            message=f"Local Ollama model '{model_name}' could not produce a safe structured response after {settings.ollama_max_attempts} attempt(s). {last_error}",
        )
    ...
    def generate_json(
        ...
        *,
        ...
        """Generate one Pydantic-constrained JSON result, with an optional local fallback model.

        Tries the configured primary model first. If it is unreachable, not installed, times
        out, or returns an HTTP error, and a local fallback model is configured, this retries
        the entire generation once against the fallback model on the same loopback Ollama
        server. This never contacts a non-local endpoint -- ``_ensure_model_ready`` and
        ``_send_chat_request`` both call ``_require_local_endpoint`` regardless of which model
        name is used. A fallback is not attempted for output-validation failures, since those
        are a JSON-contract problem already covered by the correction retry, not an
        ...
        try:
        ...
        except OllamaUnavailableError as primary_error:
            fallback_model = settings.ollama_fallback_model.strip()
            if not settings.ollama_fallback_enabled or not fallback_model or fallback_model == settings.ollama_model:
                raise

            log_event(
                level="WARNING",
                event="ollama_primary_model_unavailable_trying_local_fallback",
                primary_model=settings.ollama_model,
                fallback_model=fallback_model,
                primary_error_code=primary_error.code,
            )
            try:
                parsed, metadata = self._generate_json_with_model(
            ...
            except OllamaUnavailableError as fallback_error:
                log_event(
                    level="WARNING",
                    event="ollama_local_fallback_also_unavailable",
                    primary_model=settings.ollama_model,
                    fallback_model=fallback_model,
                    fallback_error_code=fallback_error.code,
                )
                raise OllamaUnavailableError(
                    code="ollama_all_local_models_unavailable",
                    message=(
                        f"Neither the primary local model '{settings.ollama_model}' "
                        f"({primary_error.message}) nor the local fallback model "
                        f"'{fallback_model}' ({fallback_error.message}) could generate a response."
                    ),
                ) from fallback_error

            log_event(
                level="INFO",
                event="ollama_local_fallback_model_used",
                primary_model=settings.ollama_model,
                fallback_model=fallback_model,
            )
            return parsed, metadata
